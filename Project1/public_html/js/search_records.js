    ...
    if(document.getElementById("ReturnReceipt").checked){
        ...
        $.get('return_receipt_from_record_id', {record_id: record_id, client_returner: client_returner, new_location: new_location}, function(returnedData){
            if(returnedData.includes("ERROR")){
                document.getElementById("message-box").innerHTML = "Failed to return receipt. Check input.";
                stop_loading();
            }
            else{
        ...
        $.get('return_item', {record_id: record_id, client_returner: client_returner, new_location: new_location}, function(returnedData){
            if(returnedData.includes("ERROR")){
                // do nothing
                document.getElementById("message-box").innerHTML = "Can not return record. Check input.";
            }
            else{
...
function view_more(record_id){
    ...
    $.getJSON('get_record_details', {record_id: record_id}, 
        function(returnedData){
            ...
            document.getElementById("RecordNote_dialog").innerHTML = " " + returnedData[0].record_notes;
            document.getElementById("ReceiptNote_dialog").innerHTML = " " + returnedData[0].receipt_notes;
    });    
    ...
    $.getJSON('search_records', {ReceiptId: ReceiptId, RecordId: RecordId, item_id: item_id, item_label: item_label, Borrower: Borrower, AdminCheckerId: AdminCheckerId, BorrowBeforeDate: BorrowBeforeDate, BorrowAfterDate: BorrowAfterDate, ReturnBeforeDate: ReturnBeforeDate, ReturnAfterDate: ReturnAfterDate, ItemType: ItemType, ReceiptStatus: ReceiptStatus, RecordStatus: RecordStatus, ItemStatus: ItemStatus, lower_bound: lower_bound, upper_bound: upper_bound},
        function(returnedData){                    
            ...
            for(var i = 0 ; i < returnedData.length ; i++){
                ...
                else if(returnedData[i].return_date == "NoReturn"){
                    html_code += "<tr id=\"row_"+ returnedData[i].record_id +"\" class=\"pending\"><td>"+ returnedData[i].record_id +"<\/td><td>"+ returnedData[i].receipt_id +"<\/td><td>"+ returnedData[i].item_id +"<\/td><td>"+ returnedData[i].item_label +"<\/td><td>"+ returnedData[i].item_type +"<\/td><td>"+ returnedData[i].borrower +"<\/td><td>"+ returnedData[i].borrow_date +"<\/td><td>"+ returnedData[i].expected_date +"<\/td><td><button onClick=\"view_return('"+ returnedData[i].record_id +"', '"+ returnedData[i].item_label +"', 'row_"+ returnedData[i].record_id +"')\" disabled>Return<\/button><\/td><td class=\"hide_in_print\"><button id=\"record_"+ returnedData[i].record_id +"\" onClick=\"view_more("+ returnedData[i].record_id +")\">View More<\/button><\/td><\/tr>";
                }
                else{
